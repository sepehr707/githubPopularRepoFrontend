...
import * as constants from "../constants/github.constant";
...
describe('Actions', () => {
    ...
    it('Start Loading', () => {
        const action = actions.getRepositories();
        expect(Object.keys(action)).toStrictEqual(['type']);
        expect(action.type === constants.GET_REPOSITORIES).toBe(true);
    })
    it('Fetch valid Data', () => {
        const action = actions.getRepositoriesSuccess(items);
        expect(Object.keys(action)).toStrictEqual(['type', 'payload'])
        expect(action.type === constants.GET_REPOSITORIES_SUCCESS).toBe(true);
        expect((Array.isArray(action.payload))).toBe(true);
        expect(Object.keys(action.payload[0])).toStrictEqual([
            'key',              'id',
            'name',             'git_url',
            'html_url',         'owner',
    ...
    it('Fetch invalid data', () => {
        const action = actions.getRepositoriesSuccess();
        expect(Object.keys(action)).toStrictEqual(['type', 'payload'])
        expect(action.type === constants.GET_REPOSITORIES_FAILED).toBe(true);
        expect(action.payload === 'Invalid Data').toBe(true);
    })
})